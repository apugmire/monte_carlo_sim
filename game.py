...
class Game():
    ...
    def play_game(self, rolls):
        "takes an integer parameter to specify how many times the dice should be rolled and saves the result of the roll(s) in a wide format data frame"
        
        # roll each dice in the list and store the results in a dictionary where the key is the position in the list and the value is the list of results
        outcome = {self.pieces.index(piece): piece.roll_die(rolls) for piece in self.pieces}
        
        # convert that dictionary into a dataframe where the column names are the list index and the row names are the roll number
        results = pd.DataFrame(outcome, index=[i+1 for i in range(0,rolls)])
    ...
    def show_outcome(self, form="wide"):
        ...
        if self.form == "wide":
            ## result data frame from play_game as is
